...
const StaffPengajar = ({ className, ...props }) => {
	...
	useEffect(effect, [])
	return <div {...props} id="staff-pengajar" className={`flex xl:flex-row-reverse flex-wrap ${className}`}>
		...
		{(isHome ? state.data.filter((a, i) => i < 3) : state.data).rMap(a => <div className="flex w-full xl:w-1/4 flex-col p-5">
